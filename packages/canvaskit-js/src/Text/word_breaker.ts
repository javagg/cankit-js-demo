...
export abstract class WordBreaker {
    ...
    static nextBreakIndex(text: string, index: number): number {
        return _findBreakIndex(_FindBreakDirection.forward, text, index);
    }
    ...
    static prevBreakIndex(text: string, index: number): number {
        return _findBreakIndex(_FindBreakDirection.backward, text, index);
    }
}
 ...
 */
function _isBreak(text: string | null, index: number): boolean {
    ...
    if (_isUtf16Surrogate(text.charCodeAt(index - 1))) {
        return false;
    }
...
function _isUtf16Surrogate(value: number): boolean {
...
function _isAHLetter(property: WordCharProperty): boolean {
    return _oneOf(
        property,
